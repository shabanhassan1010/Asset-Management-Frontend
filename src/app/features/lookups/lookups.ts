  ...
  save(): void {
    ...
    request.subscribe({
      next: () => {
        ...
        // Success toasts are always manual — the interceptor only knows about
        // errors, it has no idea what a successful call meant to the user.
        // We read `id` here to say "added" or "updated" with the right wording.
        // `singular` is lowercase in LOOKUP_KINDS ('category', 'assetType'),
        // so we put the name first to avoid a sentence starting lowercase.
        this.toast.success(
          `"${name.trim()}" was ${id === 0 ? 'added' : 'updated'}.`,
        );
