...
export class EmployeeService {
  ...
  list(query: EmployeeQuery): Observable<EmployeePagedResult<EmployeeRow>> {
    ...
    return this.http
      .get<ApiResponse<EmployeePagedResult<EmployeeRow>>>(API.employees.paginated, { params })
      .pipe(map(response => response.data));
  }

  getById(id: number): Observable<EmployeeDetails> {
    return this.http
      .get<ApiResponse<EmployeeDetails>>(API.employees.byId(id))
      .pipe(map(response => response.data));
  }
  ...
  setStatus(id: number, isActive: boolean): Observable<unknown> {
    return this.http
      .patch(API.employees.status(id), { isActive })
